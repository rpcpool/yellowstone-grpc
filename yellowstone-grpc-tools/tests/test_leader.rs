...
async fn test_create_leader_state_log() {
    ...
    let mut state_watch = observe_consumer_group_state(ctx.etcd.clone(), consumer_group_id)
        .await
        .unwrap();
...
async fn test_become_leader_and_resign() {
    ...
    let mut leader_observer = observe_leader_changes(ctx.etcd.clone(), consumer_group_id)
        .await
        .unwrap();
    let mut leader_resp = ctx
        .etcd
        .election_client()
        .leader(leader_key.name())
        .await
        .unwrap();
    let actual_leader_key = leader_resp.kv().unwrap().key().to_vec();
    let actual_leader_info =
        serde_json::from_slice::<LeaderInfo>(leader_resp.kv().unwrap().value()).unwrap();
    ...
    let leader_info = leader_observer
...
async fn test_leader_state_transation_during_timeline_translation() {
    ...
    assert!(matches!(
        inner_state.substate,
        TranslationState::ComputingNextProducer(_)
    ));
