...
use yellowstone_grpc_tools::{scylladb::{etcd_utils::lock::{try_lock, ManagedLock}, types::{BlockchainEvent, BlockchainEventType, CommitmentLevel, Slot}, yellowstone_log::{common::SeekLocation, consumer_group::{consumer_source::{ConsumerSource, ConsumerSourceCommand, FromBlockchainEvent}, context::ConsumerContext, etcd_path::get_producer_lock_path_v1, lock::ConsumerLocker}}}, setup_tracing};
...
async fn test_consumer_source_run() {
    ...
    let cg_info = ctx.consumer_group_store.create_static_consumer_group(
        &consumer_ids, 
    ...
    let consumer_lock = consumer_locker
        .try_lock_instance_id(consumer_group_id, &consumer_id).await.unwrap();

    let (initial_revision, shard_offset_map1) = ctx.consumer_group_store.get_shard_offset_map(
        &consumer_group_id, 
        &consumer_id, 
        &execution_id, 
    ...
    shard_offset_map_per_event_type.insert(BlockchainEventType::AccountUpdate, shard_offset_map1.clone());
    ...
    let (new_revision, shard_offset_map2) = ctx.consumer_group_store.get_shard_offset_map(
        &consumer_group_id, 
        &consumer_id, 
        &execution_id, 
        BlockchainEventType::AccountUpdate
    ).await.unwrap();
    println!("new_revision: {:?}, initial_revision: {:?}", new_revision, initial_revision);
