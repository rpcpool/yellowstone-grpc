...
use common::{MockProducerMonitor, TestContext};
use tokio::sync::{mpsc, oneshot, RwLock};
use yellowstone_grpc_tools::{scylladb::{etcd_utils::lock::{try_lock, ManagedLock}, types::{BlockchainEvent, BlockchainEventType, CommitmentLevel, Slot}, yellowstone_log::{common::SeekLocation, consumer_group::{consumer_group_store::ScyllaConsumerGroupStore, consumer_source::{ConsumerSource, ConsumerSourceCommand, FromBlockchainEvent}, context::ConsumerContext, etcd_path::get_producer_lock_path_v1, lock::ConsumerLocker, producer::{ProducerMonitor, ScyllaProducerStore}}}}, setup_tracing};
...
async fn test_get_lowest_common_slot_number() {
    ...
    let (revision,execution_id) = ctx
        .producer_store
        .get_execution_id(producer_id).await.unwrap().unwrap();
    ...
    let cg_store = ScyllaConsumerGroupStore::new(
        Arc::clone(&ctx.session),
        producer_store
    ).await.unwrap();
    let cg_info = cg_store.create_static_consumer_group(
        &consumer_ids, 
