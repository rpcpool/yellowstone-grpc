...
lazy_static::lazy_static! {
    ...
    static ref SLOT_STATUS: IntGaugeVec = IntGaugeVec::new(
        Opts::new("slot_status", "Latest received slot from Geyser"),
        &["status"]
    ).unwrap();
    ...
    static ref INVALID_FULL_BLOCKS: IntGaugeVec = IntGaugeVec::new(
        Opts::new("invalid_full_blocks_total", "Total number of fails on construction full blocks"),
        &["reason"]
    ).unwrap();
...
impl PrometheusService {
    pub async fn new(
        ...
        REGISTER.call_once(|| {
            ...
            register!(GEYSER_LOOP_ITERATION_DURATION);
            register!(FILTER_UPDATE_DURATION_VEC);
