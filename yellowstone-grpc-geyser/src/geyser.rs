use {
    crate::{
        config::ConfigBlockFailAction,
        filters::FilterAccountsDataSlice,
        prom::{self, MESSAGE_QUEUE_SIZE},
    },
    log::error,
...
#[derive(Debug, Clone)]
pub struct GeyserMessageAccountInfo {
    pub pubkey: Pubkey,
    pub lamports: u64,
    pub owner: Pubkey,
    pub executable: bool,
...
#[derive(Debug, Clone)]
pub struct GeyserMessageAccount {
    pub account: GeyserMessageAccountInfo,
    pub slot: u64,
    pub is_startup: bool,
}

impl<'a> From<(&'a ReplicaAccountInfoV3<'a>, u64, bool)> for GeyserMessageAccount {
    fn from((account, slot, is_startup): (&'a ReplicaAccountInfoV3<'a>, u64, bool)) -> Self {
        Self {
            account: GeyserMessageAccountInfo {
...
pub struct GeyserMessageSlot {
    pub slot: u64,
    pub parent: Option<u64>,
    pub status: CommitmentLevel,
}

impl From<(u64, Option<u64>, SlotStatus)> for GeyserMessageSlot {
...
#[derive(Debug, Clone)]
pub struct GeyserMessageTransactionInfo {
...
impl GeyserMessageTransactionInfo {
...
#[derive(Debug, Clone)]
pub struct GeyserMessageTransaction {
    pub transaction: GeyserMessageTransactionInfo,
    pub slot: u64,
}

impl<'a> From<(&'a ReplicaTransactionInfoV2<'a>, u64)> for GeyserMessageTransaction {
...
impl From<&ReplicaEntryInfo<'_>> for GeyserMessageEntry {
...
pub struct GeyserMessageBlock {
    ...
    pub executed_transaction_count: u64,
    pub transactions: Vec<GeyserMessageTransactionInfo>,
    pub updated_account_count: u64,
    pub accounts: Vec<GeyserMessageAccountInfo>,
    pub entries_count: u64,
    pub entries: Vec<GeyserMessageEntry>,
}

impl
    From<(
        GeyserMessageBlockMeta,
        Vec<GeyserMessageTransactionInfo>,
        Vec<GeyserMessageAccountInfo>,
        Vec<GeyserMessageEntry>,
    )> for GeyserMessageBlock
{
    fn from(
...
#[derive(Debug, Clone)]
pub struct GeyserMessageBlockMeta {
    pub parent_slot: u64,
    pub slot: u64,
    pub parent_blockhash: String,
    pub blockhash: String,
...
impl<'a> From<&'a ReplicaBlockInfoV3<'a>> for GeyserMessageBlockMeta {
...
pub enum GeyserMessage {
    Slot(GeyserMessageSlot),
    Account(GeyserMessageAccount),
    Transaction(GeyserMessageTransaction),
    Entry(GeyserMessageEntry),
    Block(GeyserMessageBlock),
    BlockMeta(GeyserMessageBlockMeta),
}
...
#[derive(Debug, Clone)]
pub struct GeyserMessageBlockRef<'a> {
    pub parent_slot: u64,
    pub slot: u64,
    ...
    pub block_height: Option<u64>,
    pub executed_transaction_count: u64,
    pub transactions: Vec<&'a GeyserMessageTransactionInfo>,
    pub updated_account_count: u64,
    pub accounts: Vec<&'a GeyserMessageAccountInfo>,
    pub entries_count: u64,
    pub entries: Vec<&'a GeyserMessageEntry>,
}

impl<'a>
    From<(
        &'a GeyserMessageBlock,
        Vec<&'a GeyserMessageTransactionInfo>,
        Vec<&'a GeyserMessageAccountInfo>,
        Vec<&'a GeyserMessageEntry>,
    )> for GeyserMessageBlockRef<'a>
{
    fn from(
...
#[derive(Debug, Clone)]
#[allow(clippy::large_enum_variant)]
pub enum GeyserMessageRef<'a> {
    Slot(&'a GeyserMessageSlot),
    Account(&'a GeyserMessageAccount),
    Transaction(&'a GeyserMessageTransaction),
    Entry(&'a GeyserMessageEntry),
    Block(GeyserMessageBlockRef<'a>),
    BlockMeta(&'a GeyserMessageBlockMeta),
}

impl<'a> GeyserMessageRef<'a> {
...
#[derive(Debug)]
struct AccountDedupInfo {
    write_version: u64,
    message_index: usize,
}

impl AccountDedupInfo {
    const fn new(write_version: u64, message_index: usize) -> Self {
        Self {
            write_version,
            message_index,
        }
...
struct SlotMessages {
    messages: Vec<Option<GeyserMessage>>, // Option is used for accounts with low write_version
    block_meta: Option<GeyserMessageBlockMeta>,
    transactions: Vec<GeyserMessageTransactionInfo>,
    accounts_dedup: HashMap<Pubkey, AccountDedupInfo>,
    entries: Vec<GeyserMessageEntry>,
    sealed: bool,
    entries_count: usize,
...
impl SlotMessages {
    fn try_seal(&mut self) -> Option<GeyserMessage> {
        if !self.sealed {
            if let Some(block_meta) = &self.block_meta {
                    ...
                    && (entries_count == 0 || self.entries.len() == entries_count)
                {
                    ...
                    for item in self.messages.iter().flatten() {
                        if let GeyserMessage::Account(account) = item {
                            accounts.push(account.account.clone());
                        }
                    }

                    let message = GeyserMessage::Block(
...
pub async fn geyser_loop(
    mut geyser_messages_rx: mpsc::UnboundedReceiver<GeyserMessage>,
    ...
    loop {
        let message = tokio::select! {
            Some(message) = geyser_messages_rx.recv() => {
                MESSAGE_QUEUE_SIZE.dec();
                message
            }
            () = &mut processed_sleep => {
                if !processed_messages.is_empty() {
                    let _ = prefilters_broadcast_tx.send((CommitmentLevel::Processed, processed_messages.into()));
        ...
        if let Some(blocks_meta_tx) = &blocks_meta_tx {
            // error for `GeyserMessage::Slot(_) | GeyserMessage::BlockMeta(_)`: unexpected token in input
            let required = matches!(message, GeyserMessage::Slot(_))
                || matches!(message, GeyserMessage::BlockMeta(_));
        ...
        match &message {
            ...
            GeyserMessage::Slot(msg)
                if processed_first_slot.is_none() && msg.status == CommitmentLevel::Processed =>
            {
                processed_first_slot = Some(msg.slot);
            }
            GeyserMessage::Slot(msg) if msg.status == CommitmentLevel::Finalized => {
                // keep extra 10 slots
                            ...
                            Some(slot) if slot < msg_slot => {
                                if let Some(slot_messages) = messages.remove(&slot) {
                                    ...
                                    if !slot_messages.sealed && slot_messages.finalized_at.is_some()
                                    {
                                        ...
                                        prom::update_invalid_blocks(format!(
        ...
        if !matches!(message, GeyserMessage::Slot(_)) {
            ...
            if slot_messages.sealed
                && !(matches!(message, GeyserMessage::Entry(_)) && slot_messages.entries_count == 0)
            {
                prom::update_invalid_blocks(format!("unexpected message {}", message.kind()));
                match block_fail_action {
                    ConfigBlockFailAction::Log => {
                        error!(
                            "unexpected message #{} -- {} (invalid order)",
                            message.get_slot(),
                            message.kind()
                        );
                    }
        ...
        match &message {
            ...
            GeyserMessage::Account(msg) => {
                ...
                if let Some(entry) = slot_messages.accounts_dedup.get_mut(&msg.account.pubkey) {
                    if entry.write_version < write_version {
                        // We can replace the message, but in this case we will lose the order
                        slot_messages.messages[entry.message_index] = None;
                        *entry = AccountDedupInfo::new(write_version, msg_index);
                    }
                } else {
                    slot_messages.accounts_dedup.insert(
                        msg.account.pubkey,
                        AccountDedupInfo::new(write_version, msg_index),
                    );
                }
            }
            GeyserMessage::Entry(msg) => {
        ...
        for message in messages_vec {
            if let GeyserMessage::Slot(slot) = message {
                ...
                if matches!(message, GeyserMessage::Block(_)) {
