...
pub const PUBKEY_HASHER_WINDOWS: usize = 25;

pub fn pubkey_set_with_window(
    offset: usize,
...
pub fn window_colliding_pubkeys(n: usize) -> Vec<Pubkey> {
    (0..n)
        .map(|i| {
            let mut bytes = [0u8; 32];
            bytes[0] = i as u8;
...
pub fn message_transaction(
    ...
    Arc::new(MessageTransaction {
        transaction: MessageTransactionInfo {
            ...
            meta,
            index: 1,
            account_keys: account_keys.into_iter().collect::<PubkeyHashSet>(),
            pre_encoded: OnceLock::new(),
