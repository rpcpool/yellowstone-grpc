...
fn decode_slot(bytes: &[u8]) -> Result<GeyserMessage, DecodeError> {
    ...
    let status = match proto::SlotStatus::try_from(p.status).unwrap_or(proto::SlotStatus::SlotProcessed) {
    ...
    Ok(GeyserMessage::Slot(yellowstone_shmem_common::MessageSlot {
        slot: p.slot, parent: p.parent, status,
    }))
}
...
fn decode_account(bytes: &[u8]) -> Result<GeyserMessage, DecodeError> {
    let p = proto::SubscribeUpdateAccount::decode(bytes)
        .map_err(|e| DecodeError::DecodeError(e.to_string()))?;
    let info = p.account.ok_or_else(|| DecodeError::DecodeError("missing account".into()))?;
    Ok(GeyserMessage::Account(yellowstone_shmem_common::MessageAccount {
...
fn decode_entry(bytes: &[u8]) -> Result<GeyserMessage, DecodeError> {
    let p = proto::SubscribeUpdateEntry::decode(bytes)
        .map_err(|e| DecodeError::DecodeError(e.to_string()))?;
    Ok(GeyserMessage::Entry(yellowstone_shmem_common::MessageEntry {
        slot:                       p.slot,
        index:                      p.index as usize,
...
fn decode_block_meta(bytes: &[u8]) -> Result<GeyserMessage, DecodeError> {
    let p = proto::SubscribeUpdateBlockMeta::decode(bytes)
        .map_err(|e| DecodeError::DecodeError(e.to_string()))?;
    Ok(GeyserMessage::BlockMeta(yellowstone_shmem_common::MessageBlockMeta {
    ...
    pub fn to_dm_message(msg: GeyserMessage) -> Result<Message, String> {
        ...
        match msg {
            ...
            GeyserMessage::Account(a) => {
                let pubkey = Pubkey::try_from(a.account.pubkey.as_slice())
                    .map_err(|_| "invalid pubkey")?;
                let owner = Pubkey::try_from(a.account.owner.as_slice())
                    .map_err(|_| "invalid owner")?;
                let txn_signature = a.account.txn_signature
            ...
            GeyserMessage::Transaction(t) => {
                ...
                let account_keys: HashSet<Pubkey> = t.transaction.account_keys
                    .iter()
            ...
            GeyserMessage::Entry(e) => {
                ...
                let hash = Hash::new_from_array(
                    <[u8; 32]>::try_from(e.hash.as_slice())
...
fn convert_slot_status(status: &SlotStatus) -> crate::plugin::message::SlotStatus {
