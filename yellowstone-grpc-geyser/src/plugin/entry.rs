...
impl GeyserPlugin for Plugin {
    ...
    // Opt in to address lookup table resolution so deshred transactions carry
    // their loaded addresses, which the filters match on. Stock agave 4.1 leaves
    // this off by default; the triton fork resolved unconditionally. V1
    // transactions have no lookup tables, so this only affects V0.
