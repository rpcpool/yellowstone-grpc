...
mod tests {
    ...
    // These tests cover deshred routing only. Deshred is a pre-commitment
    // stream (emitted before Processed) and is independent of commitment
    // levels, so main channel commitment replay is out of scope here: that is
    // block machine territory and requires BlockMeta to seal blocks.
    mod geyser_loop_routing {
        ...
        #[tokio::test]
        async fn transaction_stays_on_main_and_off_deshred() {
            // Deshred routing must not divert normal Transaction messages:
            // they keep flowing on the main channel (immediate Processed
            // delivery) and never appear on the deshred channel. Commitment
            // replay belongs to the block machine and is out of scope here.
            let mut harness = spawn_loop();
            ...
            for (commitment, batch) in &main_batches {
                if matches!(commitment, CommitmentLevel::Processed) {
                    processed += count_transaction(batch);
                }
            }
