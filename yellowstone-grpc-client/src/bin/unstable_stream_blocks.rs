...
async fn main() -> anyhow::Result<()> {
    ...
    blocks_meta.insert(RESERVED_FILTER_NAME.to_owned(), SubscribeRequestFilterBlocksMeta::default());
    ...
    entry.insert(RESERVED_FILTER_NAME.to_owned(), SubscribeRequestFilterEntry::default());
    ...
    let boxed_stream = auto_reconnect.boxed();


    // Feed into BlockStream 
    let mut block_stream = BlockStream::new(boxed_stream, CommitmentLevel::Confirmed);
