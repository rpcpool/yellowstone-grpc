...
use yellowstone_grpc_proto::{geyser::subscribe_update::UpdateOneof, prelude::{Transaction, *}, tonic::codec::CompressionEncoding};
use base64::{engine::general_purpose, Engine as _};
use std::collections::HashMap;
use crate::bindings::{JsChannelOptions, JsSubscribeRequest, JsCompressionAlgorithm};
use std::time::Duration;
use yellowstone_grpc_client::{GeyserGrpcBuilder, ClientTlsConfig};
...
pub async fn get_client_builder(endpoint: String, x_token: Option<String>, channel_options: Option<JsChannelOptions>) -> Result<GeyserGrpcBuilder> {
  // Endpoint.
  ...
  builder = builder
    .connect_timeout(Duration::from_millis(default_opts.grpc_connect_timeout
            .expect("connect_timeout is defined in Default")
            .into()
        ))
    .timeout(Duration::from_millis(default_opts.grpc_timeout
    ...
    .max_decoding_message_size(default_opts.grpc_max_decoding_message_size
        .expect("max_decoding_message_size is defined in Default")
        as usize
        )
    .max_encoding_message_size(default_opts.grpc_max_encoding_message_size
        .expect("max_encoding_message_size is defined in Default")
        as usize
        )
    .http2_adaptive_window(default_opts.grpc_http2_adaptive_window
        .expect("grpc_http2_adaptive_window is defined in Default"))
    .tcp_nodelay(default_opts.grpc_tcp_nodelay
...
fn to_js_message_header(env: &Env, header: MessageHeader) -> Result<Object> {
    let mut obj = Object::new(env)?;
    obj.set("numRequiredSignatures", header.num_required_signatures as u32)?;
    obj.set("numReadonlySignedAccounts", header.num_readonly_signed_accounts as u32)?;
    obj.set("numReadonlyUnsignedAccounts", header.num_readonly_unsigned_accounts as u32)?;
    Ok(obj)
}
...
fn to_js_block(env: &Env, block: SubscribeUpdateBlock) -> Result<Object> {
    ...
    obj.set("executedTransactionCount", block.executed_transaction_count.to_string())?;
    obj.set("updatedAccountCount", block.updated_account_count.to_string())?;
...
fn to_js_entry(env: &Env, entry: SubscribeUpdateEntry) -> Result<Object> {
    ...
    obj.set("executedTransactionCount", entry.executed_transaction_count.to_string())?;
    obj.set("startingTransactionIndex", entry.starting_transaction_index.to_string())?;
    Ok(obj)
}
...
fn to_js_block_meta(env: &Env, meta: SubscribeUpdateBlockMeta) -> Result<Object> {
    ...
    obj.set("executedTransactionCount", meta.executed_transaction_count.to_string())?;
    ...
    if let Some(oneof) = update.update_oneof {
        match oneof {
            ...
            UpdateOneof::TransactionStatus(tx_status) => {
                obj.set("transactionStatus", to_js_transaction_status(env, tx_status)?)?;
            }
        }
    }
